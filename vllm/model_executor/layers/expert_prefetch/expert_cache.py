...
class CacheStats:
    ...
    staged: int = 0
    layers: int = 0
    # Prefetches dropped because the worker was still busy with the previous
    # one. Each is a layer that falls back entirely to fetch-on-demand.
    skipped: int = 0
...
class AccuracyTracker:
    ...
    def log(self) -> None:
        ...
        summary = (
            ...
            f"needed={sum(self._needed.values())}) | {per_layer}"
        )
    ...
    def _prefetch_worker(
        ...
        try:
            ...
            with torch.cuda.stream(stream):
                ...
                # Ported from offload_vllm's `chunking_prefetch`: stage the very
                # first expert on its own and synchronize before issuing the
                # rest. The lone leading copy settles the buffer (and warms the
                # H2D path) so the bulk copies that follow land cleanly; a final
                # sync makes the whole batch complete before the event fires.
                if n:
                    buf.copy_expert(owner, 0, expert_list[0])
                    stream.synchronize()
                    for slot_id in range(1, n):
                        buf.copy_expert(owner, slot_id, expert_list[slot_id])
                    stream.synchronize()
